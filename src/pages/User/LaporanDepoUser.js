...
export default function LaporanPageDepo() {
  ...
  const getPetiKemas = async () => {
    const response = await axios.get('http://localhost:3001/petiKemas/show', {
      headers: {
    ...
    <>
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Link to="/dashboard-user/tambah-laporan-depo-user">
                ...
                <tbody>
                  {petiKemas.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td>{row.surat}</td>
                      <td>
                        <Link to={`/dashboard-user/edit-laporan-depo-user/${row.id}`}>
